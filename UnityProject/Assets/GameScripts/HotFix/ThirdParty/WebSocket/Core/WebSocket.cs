...
namespace NativeWebSocket
{
  ...
  public class WebSocket : IWebSocket {
    ...
    public class WebSocket : IWebSocket
    {
        ...
        public WebSocketCloseStatus? CloseStatus => m_Socket?.CloseStatus;
        public ClientWebSocket ClientWebSocket=>m_Socket;
        ...
        public async Task Connect(X509Certificate2 certificate = null)
        {
            try
            {
                ...
                if (certificate != null)
                {
                    m_Socket.Options.ClientCertificates.Add(certificate); //=x59;
                    m_Socket.Options.UseDefaultCredentials = true;
                    m_Socket.Options.RemoteCertificateValidationCallback = OnRemoteCertificateValidationCallback;
                }
        ...
        private bool OnRemoteCertificateValidationCallback(object sender, X509Certificate certificate, X509Chain chain, SslPolicyErrors sslPolicyErrors)
        {
            ...
            Debug.Log($"Certificate Issuer: {certificate.Issuer}");
            Debug.Log($"Certificate Thumbprint: {certificate.GetCertHashString()}");
            Debug.Log($"Certificate Valid From: {certificate.GetEffectiveDateString()}");
            Debug.Log($"Certificate Valid To: {certificate.GetExpirationDateString()}");

            // 输出证书链信息
            if (chain != null)
            {
                Debug.Log("Certificate Chain:");
                foreach (X509ChainElement element in chain.ChainElements)
                {
                    Debug.Log($"  Chain Element Subject: {element.Certificate.Subject}");
                    Debug.Log($"  Chain Element Issuer: {element.Certificate.Issuer}");
                    Debug.Log($"  Chain Element Thumbprint: {element.Certificate.GetCertHashString()}");
                }
            }
        ...
        public void SetPingInterval(TimeSpan timeSpan)
        {
