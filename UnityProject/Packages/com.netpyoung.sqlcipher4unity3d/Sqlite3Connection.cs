...
namespace SqlCipher4Unity3D
{
	...
	public partial class SQLiteConnection : IDisposable
	{
		...
		public bool IsTableExist<T>(CreateFlags createFlags = CreateFlags.None)
		{
			var ty = typeof(T);
			var map = GetMapping(ty, createFlags);
			...
			if (map.Columns.Length == 0)
			{
				throw new Exception(string.Format(
					"Cannot Get a table without columns (does '{0}' have public properties?)", ty.FullName));
			}

			var existingCols = GetTableInfo(map.TableName);
			return existingCols.Count > 0;
		}
		...
		public int DeleteAll<T>(IEnumerable<T> objects, bool runInTransaction = true)
		{
			var c = 0;
			if (runInTransaction)
			{
